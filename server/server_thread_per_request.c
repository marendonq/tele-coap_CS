...
void *process_message(void *arg)
{
    ...
    if (parse_coap_message(buffer, n, &request) == 0)
    {
        ...
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "Mensaje CoAP parseado - Ver:%d Tipo:%d Código:%d", request.ver, request.type, request.code);
        logger_log(logger, log_msg);
        ...
        if (coap_router_handle_request(&request, &resp_code, response_payload, sizeof(response_payload)) == 0)
        {
            ...
            // Si el mensajes es un NON no se responde
            ...
            else
            {
                ...
                char log_msg2[256];
                snprintf(log_msg2, sizeof(log_msg2), "Respuesta omitida mensaje NON");
                logger_log(logger, log_msg2);
            }
        }
